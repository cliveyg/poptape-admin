...
func TestBackupMongo_HappyPath(t *testing.T) {
	...
	err = a.BackupMongo(args)
	require.NoError(t, err)
	require.Equal(t, int64(42), bytesWritten)
}

func TestBackupMongo_CreateGridFSUploadStreamError(t *testing.T) {
	a, _, hooks := testutils.SetupAppWithMockDBAndHooks(t)
	origEncrypt := utils.Encrypt
	origDecrypt := utils.Decrypt
	utils.Encrypt = func(pw, key, nonce []byte) (string, error) { return "mocked_encrypted_password", nil }
	...
	cred := &app.Cred{CredId: uuid.New(), DBName: "testdb", Host: "localhost", Type: "mongo", URL: "/testdb", DBPort: "27017", DBUsername: "user", DBPassword: encPW}
	msID := uuid.New()
	saveID := uuid.New()
	var bytesWritten int64
	args := &app.BackupDBArgs{
...
func TestBackupMongo_CopyToGridFSError(t *testing.T) {
	a, _, hooks := testutils.SetupAppWithMockDBAndHooks(t)
	origEncrypt := utils.Encrypt
	origDecrypt := utils.Decrypt
	utils.Encrypt = func(pw, key, nonce []byte) (string, error) { return "mocked_encrypted_password", nil }
...
func TestBackupMongo_SaveWithAutoVersionError(t *testing.T) {
	a, _, hooks := testutils.SetupAppWithMockDBAndHooks(t)
	origEncrypt := utils.Encrypt
	origDecrypt := utils.Decrypt
	utils.Encrypt = func(pw, key, nonce []byte) (string, error) { return "mocked_encrypted_password", nil }
...
func TestBackupMongo_DecryptPasswordError(t *testing.T) {
	a, _, hooks := testutils.SetupAppWithMockDBAndHooks(t)
	origEncrypt := utils.Encrypt
	origDecrypt := utils.Decrypt
