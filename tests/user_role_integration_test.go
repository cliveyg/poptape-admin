...
func TestLogin_DBSaveError(t *testing.T) {
	resetDB(t, TestApp)
	...
	// Create a new user
	userUsername := "logindberr_" + RandString(8)
	...
	// Directly update the user's username in the DB to "" (empty string) to violate NOT NULL/unique constraint
	result := TestApp.DB.Model(&app.User{}).Where("username = ?", userUsername).Update("username", "")
	require.NoError(t, result.Error)

	// Attempt login as the now-invalid user (should hit .Save and fail)
	loginReq := map[string]string{
		"username": "",
		"password": base64.StdEncoding.EncodeToString([]byte(userPassword)),
	}
	...
	TestApp.Router.ServeHTTP(w2, req2)
	require.Equal(t, http.StatusInternalServerError, w2.Code)
	require.Contains(t, w2.Body.String(), "Ooops")
}
